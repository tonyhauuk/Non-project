...
def show_student(lst):
    if len(lst) == 0:
        print('没有查询到学生信息，无数据现实')
        return
    # 定义标题的显示格式
    format_title = '{:"6}\t{:"12}\t{:"8}\t{:"10}\t{:"10}|t{:"8}'
    print(format_title.format('ID', '姓名', '英语成绩', 'Python成绩', 'Java成绩', '总成绩'))
    # 定义内容显示格式
    format_data = '{:"6}\t{:"12}\t{:"8}\t{:"8}\t{:"8}|t{:"8}'
    for item in lst:
...
def sort():
    ...
    if mode == '1':
        ...
        student_new.sort(key = lambda student_new: int(student_new['english']) + int(student_new['python']) + int(
            student_new['java']), reverse = asc_or_desc_bool)
    else:
        print('您的输入有误请重新输入')
        sort()
